...
void init_gps_uart(void) {
  const uart_config_t uart_config = {
      .baud_rate = UBX_RATE, // M10 默认黄金波特率
  ...
  ESP_ERROR_CHECK(uart_driver_install(GPS_UART_NUM, BUF_SIZE * 2, BUF_SIZE * 2,
                                      0, NULL, 0));
...
void gps_ubx_task(void *pvParameters) {
  ...
  while (1) {
    ...
    int len = uart_read_bytes(GPS_UART_NUM, buffer, BUF_SIZE - 1,
